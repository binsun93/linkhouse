...
class MY_Lang extends CI_Lang {
    ...
    function MY_Lang() {
        ...
        $dmm = explode('/',trim($_SERVER['REQUEST_URI'],'/'));
        if (($url_ok) && ($this->is_special($dmm[0]))) { // special URI -> redirect
            // set default language
            $CFG->set_item('language', $this->languages[$this->default_lang()]);
            $uri = (!empty($this->uri)) ? $this->uri : $this->default_uri;
            $fixUri = substr($uri, 2);
            $new_url = $CFG->config['base_url'] . $fixUri;
    ...
    function switch_uri($lang) {
        ...
        if ((!empty($this->uri)) && (array_key_exists($lang, $this->languages))) {
            $uri_segment = $CI->uri->segments;
            if (!empty($uri_segment)) {
                $uri = $lang . '/' . $uri_segment[2];
            } else {
                $uri = $lang . '/' . $this->uri->rsegments[1];
            }
        }
        return $uri;
    }
